...
- S16_LE
```
Обращаем внимание на параметр RATE: 192000 - это частота дискретизации 192 кГц. У второго устройства в моем случае частота дискретизации составляет 48 кГц. Значит данное устройство `hw:3,1` (или более детально `hw:CARD=reciever,DEV=1`) похоже на то, что мы ищем для записи IQ сигнала.

### Производим запись IQ сигнала
Настраиваем приемник на необходимую нам частоту радиостанции, контролируем по спектрограмме и водопаду наличие полезного сигнала.
...
while true; do
    TIMESTAMP=$(date +%s)
    FILENAME="rec/iq_${TIMESTAMP}.$FORMAT"
    # Запись файла
