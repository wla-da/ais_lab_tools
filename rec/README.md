...
- [Запись IQ сигнала с приемника Малахит DSP3](#запись-iq-сигнала-с-приемника-малахит-dsp3)
  - [Способы](#способы)
    - [1. Запись с помощью ALSA](#1-запись-с-помощью-alsa)
    - [2. Запись с помощью GRC (GNU Radio)](#2-запись-с-помощью-grc-gnu-radio)
    - [3. Запись с помощью SDR программ](#3-запись-с-помощью-sdr-программ)
  - [Подключить приемник к компьютеру через USB](#подключить-приемник-к-компьютеру-через-usb)
  - [0. Общее. Определить устройство для захвата IQ сигнала с приемника](#0-общее-определить-устройство-для-захвата-iq-сигнала-с-приемника)
  - [1. Запись IQ сигнала с помощью ALSA](#1-запись-iq-сигнала-с-помощью-alsa)
  - [2. Запись IQ сигнала с помощью GRC (GNU Radio)](#2-запись-iq-сигнала-с-помощью-grc-gnu-radio)
  - [3. Запись IQ сигнала с помощью SDR программ](#3-запись-iq-сигнала-с-помощью-sdr-программ)
  - [4. Запись IQ сигнала с помощью спектроанализатора Rigol RSA3015N](#4-запись-iq-сигнала-с-помощью-спектроанализатора-rigol-rsa3015n)
...
### 4. Запись IQ сигнала с помощью спектроанализатора Rigol RSA3015N
Бонус для тех, кто дочитал.
В самом начале изысканий, когда только начинал обсуждать с LLM задачу захвата IQ сигнала, LLM утверждали, что с помощью спектроанализатора Rigol RSA3015N можно захватить IQ сигнал.
Как оказалось впоследствии - это не так: можно сохранить скриншоты, данные измерений, состояние прибора, маску захвата сигнала по триггеру и тп, но не сам IQ сигнал. 

Кажется, что всё таки это можно сделать, но придется лезть с "паяльником" внутрь прибора, но этого пока не хочется делать. Поэтому просто любуемся на сигнал AIS на экране RSA3015N:
![RSA3015N. AIS сигнал](img/rsa3015n_ais.bmp)
